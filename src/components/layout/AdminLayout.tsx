...
import { FiBarChart2, FiBookOpen, FiCreditCard, FiLogOut, FiMessageSquare, FiSettings, FiTag, FiTarget, FiUpload, FiUsers } from 'react-icons/fi';
import headerLogo from '../../assets/header-logo.png';
import { useAuth } from '../../context/AuthContext';

const adminNav = [
  { to: '/admin', label: 'Overview', icon: FiBarChart2, end: true },
  { to: '/admin#auth-settings', label: 'Auth Settings', icon: FiSettings },
  ...
  { to: '/admin#company-exams', label: 'Company Exams', icon: FiTarget },
...
export const AdminLayout = ({ children }: { children: ReactNode }) => {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      ...
      <aside className="fixed inset-y-0 left-0 z-50 hidden w-60 flex-col overflow-hidden border-r border-[var(--border)] bg-[var(--surface)] lg:flex">
        <Link to="/" className="flex shrink-0 items-center gap-2 border-b border-[var(--border)] px-5 py-5">
          ...
          <div className="mt-auto border-t border-[var(--border)] bg-[var(--surface)] p-4 space-y-3">
        ...
        {mobileOpen && (
          <div className="fixed inset-0 z-[100] lg:hidden">
            ...
            <motion.aside
              ...
              className="absolute left-0 top-0 flex h-full w-[min(320px,88vw)] flex-col overflow-hidden border-r border-[var(--border)] bg-[var(--surface)] shadow-2xl"
            >
