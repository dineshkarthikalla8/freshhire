...
export const TopicGridCard = ({
  ...
  const { user, authSettings } = useAuth();
  const isLocked = authSettings.pricingMode === 'paid' && !user?.hasPaid;
  ...
  return (
    ...
    >
      ...
      <div className="mt-auto flex items-center justify-between gap-3 pt-5">
        ...
        >
          Open {isLocked ? <FiLock className="h-4 w-4" /> : <FiArrowRight className="h-4 w-4" />}
        </Link>
      </div>
    </motion.article>
  );
