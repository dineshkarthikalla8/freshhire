...
import { verbalTopics, panelAccent } from '../data/panels';
        ...
        <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-[2rem] border border-[var(--border)] bg-[var(--card)] p-8 shadow-xl">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              <p className="text-xs font-black uppercase tracking-[0.35em] text-[var(--muted-foreground)]">Verbal</p>
              <h1 className="mt-3 text-4xl md:text-5xl font-black tracking-tight text-[var(--foreground)]">Formal verbal practice with exam-ready examples and notes.</h1>
              <p className="mt-4 text-lg text-[var(--muted-foreground)] leading-relaxed">Every lesson is static and preloaded so the site remains light on free hosting, while still covering grammar, vocabulary, reading comprehension, and coherence tasks in detail.</p>
            </div>
            <div className="grid grid-cols-2 gap-3 text-sm font-bold text-[var(--foreground)]">
              <div className="rounded-2xl border border-[var(--border)] bg-[var(--background)] px-4 py-3">Grammar</div>
              <div className="rounded-2xl border border-[var(--border)] bg-[var(--background)] px-4 py-3">RC</div>
              <div className="rounded-2xl border border-[var(--border)] bg-[var(--background)] px-4 py-3">Vocabulary</div>
              <div className="rounded-2xl border border-[var(--border)] bg-[var(--background)] px-4 py-3">Para jumbles</div>
            </div>
          </div>
        </motion.section>
          ...
          {verbalTopics.map((topic, index) => (
            ...
            >
              ...
              <div className="mt-5 space-y-4">
                ...
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.3em] text-[var(--muted-foreground)]">Formal examples</p>
