...
import { aptitudeTopics, panelAccent } from '../data/panels';
        ...
        <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-[2rem] border border-[var(--border)] bg-[var(--card)] p-8 shadow-xl">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              <p className="text-xs font-black uppercase tracking-[0.35em] text-[var(--muted-foreground)]">Aptitude</p>
              <h1 className="mt-3 text-4xl md:text-5xl font-black tracking-tight text-[var(--foreground)]">High-signal quantitative practice for campus hiring.</h1>
              <p className="mt-4 text-lg text-[var(--muted-foreground)] leading-relaxed">These modules are preloaded for free hosting and focus on the exact topics that recur in aptitude rounds: arithmetic, algebra, probability, geometry, time-work, and data interpretation.</p>
            </div>
            <div className="grid grid-cols-2 gap-3 text-sm font-bold text-[var(--foreground)]">
              <div className="rounded-2xl border border-[var(--border)] bg-[var(--background)] px-4 py-3">Core topics</div>
              <div className="rounded-2xl border border-[var(--border)] bg-[var(--background)] px-4 py-3">Static content</div>
              <div className="rounded-2xl border border-[var(--border)] bg-[var(--background)] px-4 py-3">Formal shortcuts</div>
              <div className="rounded-2xl border border-[var(--border)] bg-[var(--background)] px-4 py-3">Exam style notes</div>
            </div>
          </div>
        </motion.section>
          ...
          {aptitudeTopics.map((topic, index) => (
            ...
            >
              ...
              <div className="mt-5 space-y-4">
                ...
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.3em] text-[var(--muted-foreground)]">Exam-style examples</p>
