...
const toLeetCodeSlug = (title: string) => {
  const override = LEETCODE_SLUG_OVERRIDES[title];
...
for (const line of rawProblems.split('\n')) {
  ...
  const difficulty = trimmed.slice(separatorIndex + 1) as Problem['difficulty'];
  ...
  problems.push({
    ...
    difficulty,
    platform: 'LeetCode',
    url: `https://leetcode.com/problems/${toLeetCodeSlug(title)}/`,
  });
