...
2. Set Razorpay credentials for the backend (do NOT commit secrets):

```bash
firebase functions:config:set razorpay.key_id="your_key_id" razorpay.key_secret="your_key_secret"
```

3. Deploy (build + deploy backend and hosting):
...
Notes:
- The backend endpoints `/createOrder`, `/verifyPayment`, and `/extract-text` are proxied via `firebase.json` rewrites to the `api` function.
